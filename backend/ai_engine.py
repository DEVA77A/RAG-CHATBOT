...
def validate_retrieved_chunks(question: str, context_chunks: list[dict], title: str, url: str) -> dict:
    ...
    Bypassed pre-validation LLM call to save rate limit quota and speed up chat.
    """
    return {
...
async def rag_chat(
    ...
    **kwargs,  # Accept extra kwargs for backward compat
    ...
    # Extract cited sources (robust to multi-citations and general formatting)
    sources = []
    cited_ids = set(int(num) for num in re.findall(r"Source\s*(\d+)", answer))
    ...
    for i, chunk in enumerate(filtered_chunks, 1):
        ...
        if chunk_id in cited_ids or i in cited_ids:
            source_url = meta.get("source_url") or meta.get("url") or url
            source_title = meta.get("source_title") or meta.get("page_title") or title or "Indexed Content"
            if not source_title.strip() or source_title.strip() == "Unknown Title":
                source_title = title or "Indexed Content"
    ...
    # Clean up all inline citations (e.g. [Source 12], [Source 0, Source 1], [Source: Title](url))
    # This regex matches any brackets containing the word "Source", along with any trailing URL parenthesis.
