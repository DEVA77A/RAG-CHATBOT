...
async def analyze_endpoint(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    """
    Analyze a website for a specific persona.

    Pipeline: Scrape → Chunk → Embed → Store in FAISS → Mega-prompt → Return.
    Runs synchronously for simplicity (hackathon). Typical time: 10-20 seconds.
    """
    ...
    logger.info(f"[{analysis_id}] Starting analysis: {url} (persona: {persona})")
    ...
    metadata = [{"chunk_index": i, "url": url} for i in range(len(chunks))]
    store.add(chunks=chunks, embeddings=embeddings, metadata=metadata)
    store.save()
...
async def chat_endpoint(request: ChatRequest):
    ...
    return ChatResponse(
        ...
        sources=[
            ChatSource(content=s["content"], score=s["score"])
